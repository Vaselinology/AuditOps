...
function generateNotificationHtml(audit, formData, version) {
  const today = formData.date ? new Date(formData.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
  ...
  const auditPlanRows = formData.auditPlan && formData.auditPlan.length > 0 
    ? formData.auditPlan.map(item => `
      <tr>
  ...
  <table class="info-table">
    ...
    <tr>
      ...
      <td>
        ...
        ${formData.location || audit.audit_place || ''}
      </td>
    </tr>
    <tr><td>Type d'audit</td><td>${formData.type || audit.type || 'Planifié'}</td></tr>
    <tr><td>Objet de l'audit</td><td>${audit.title || '—'}</td></tr>
    <tr><td>Durée de l'audit</td><td>${formData.duration || '—'}</td></tr>
    <tr><td>Lieu / Département</td><td>${audit.department || '—'}</td></tr>
    <tr><td>Référentiels appliqués</td><td>${formData.referentiels || version}</td></tr>
