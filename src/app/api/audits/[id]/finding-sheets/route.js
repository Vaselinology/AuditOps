...
export async function POST(request, { params }) {
  try {
    console.log("Finding sheets generation request received for audit:", params.id);
    const { id } = params;
    const body = await request.json();
    console.log("Finding sheets request body:", body);
...
<div class="page">
  ...
  <div class="footer">
    ...
    for (const version of ['DGAC', 'PART']) {
      console.log(`Inserting finding sheet for version ${version}`);
      const { error: insertError } = await supabaseServer
        .from('finding_sheets')
        .insert({
          audit_id: id,
          version,
          html_content: generateFindingSheetHTML(version),
          created_at: new Date().toISOString()
        });
      
      if (insertError) {
        console.error(`Error inserting finding sheet ${version}:`, insertError);
      } else {
        console.log(`Successfully inserted finding sheet ${version}`);
      }
    }

    return Response.json({
      success: true,
      message: "Finding sheets generated successfully",
      audit_number: audit.audit_number,
    });
  } catch (error) {
    console.error("Error generating finding sheets:", error);
    return Response.json(
      { error: "Failed to generate finding sheets" },
      { status: 500 },
    );
  }
}
